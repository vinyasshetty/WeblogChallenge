...
object WebLogMain_ignore_ms {

  /*
    This function cleans up the raw_data.
    Converting the timestamp values to only have information until seconds ie (yyyy-MM-dd HH:mm:ss),
    since i think it was not necessary to use microseconds when evaluating sessions time and also to avoid
    potential precision issues.Also had to explicitly give the timestamp format since timestamp was getting shifted when
    i did not provide it explicitly.
  ...
  def standardize(spark:SparkSession, raw_data:DataFrame):DataFrame={
    ...
    val std_data = raw_data.select(
      ...
      .withColumn("timediff", when($"prevts".isNull,0).otherwise(($"ts".cast("long") - $"prevts".cast("long"))/60.0).cast("decimal(12,4)"))
  ...
  def main(args: Array[String]): Unit = {
     ...
     */
    sessioned_data.groupBy($"sessionid",$"clientip")
      .agg(((max($"ts").cast("long") - min($"ts").cast("long"))/60).cast("decimal(8,2)").as("sessiontime"))
      .select(avg($"sessiontime").as("avgsession(in mins)")).show()

    //Total Number of Sessions -- 113378
